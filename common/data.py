...
def load_dataset(name):
    ...
    if name == "enzymes":
        ...
        dataset = GINDataset(name="PROTEINS",self_loop=False)
    elif name == "reddit-binary":
        dataset = TUDataset(name="REDDITBINARY")
...
class DiskDataSource(DataSource):
    ...
    def gen_batch(self, a, b, c, train, max_size=15, min_size=5, seed=None,
        ...
        pos_a = utils.batch_dgl_graphs(pos_a, anchors=pos_a_anchors if
            self.node_anchored else None)
        pos_b = utils.batch_dgl_graphs(pos_b, anchors=pos_b_anchors if
            self.node_anchored else None)
        neg_a = utils.batch_dgl_graphs(neg_a, anchors=neg_a_anchors if
            self.node_anchored else None)
        neg_b = utils.batch_dgl_graphs(neg_b, anchors=neg_b_anchors if
            self.node_anchored else None)
        return pos_a, pos_b, neg_a, neg_b
