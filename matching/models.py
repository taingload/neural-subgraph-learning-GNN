...
class SkipLastGNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, args):
        ...
        for l in range(self.n_layers):
            ...
            if args.conv_type == "PNA":
                ...
                self.convs.append(conv_model(hidden_input_dim, hidden_dim,"mean"))

        post_input_dim = hidden_dim * (self.n_layers + 1)
        if args.conv_type == "PNA":
            post_input_dim *= 3
    ...
    def build_conv_model(self, model_type, n_inner_layers):
        if model_type == "GCN":
            return dglconv.GCNConv
        elif model_type == "GIN":
            #return lambda i, h: pyg_nn.GINConv(nn.Sequential(
            #    nn.Linear(i, h), nn.ReLU()))
            return lambda i, h: dglconv.GINConv(nn.Sequential(
                nn.Linear(i, h), nn.ReLU(), nn.Linear(h, h)
                ))
        elif model_type == "SAGE":
            return dglconv.SAGEConv
        elif model_type == "graph":
            return dglconv.GraphConv
        elif model_type == "GAT":
            return dglconv.GATConv
        elif model_type == "gated":
            return lambda i, h: dglconv.GatedGraphConv(h, n_inner_layers)
        elif model_type == "PNA":
            return dglconv.SAGEConv
        else:
    ...
    def forward(self, data):
        ...
        for i in range(len(self.convs_sum) if self.conv_type=="PNA" else
            ...
            if self.skip == 'learnable':
                ...
                if self.conv_type == "PNA":
                    x = torch.cat((self.convs_sum[i](data,curr_emb),
                        self.convs_mean[i](data,curr_emb),
                        self.convs_max[i](data,curr_emb)), dim=-1)
                else:
                    x = self.convs[i](data,curr_emb)
            elif self.skip == 'all':
                if self.conv_type == "PNA":
                    x = torch.cat((self.convs_sum[i](data,curr_emb),
                        self.convs_mean[i](data,curr_emb),
        ...
        emb = torch.sum(emb,0).unsqueeze(0) # dglconv.global_add_pool(emb, batch)
    ...
    def loss(self, pred, label):
        return F.nll_loss(pred, label)
'''
class SAGEConv(dglconv.MessagePassing):
